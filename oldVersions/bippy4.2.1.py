#Aaron Perkel
#Bippy V4.2.1 (02/22/21 21:03:47PM)
...
pg.display.set_caption('Bippy V4.2.1') # window title
pg.init() # initialize pygame
startTime = time.time() # time for warning screen DELETE IN FINAL VERSION
...
class Object: # class to handle objects
    def __init__(self, inputScreen): # initialize class
        ...
        self.xLoc = random.randint(660, 1260) # object xLoc
...
titleScreen = False # booleans
gameScreen = False
isJump = False
enterScreen = False
endScreen = False
controlsScreen = False
warningScreen = True # DELETE FOR FINAL VERSION
...
def resetVars(): 
    ...
    global bg1XLoc, bg2XLoc, objects, score
...
while True:
    ...
    while warningScreen: # DELETE IN FINAL VERSION
        screen.fill((255, 255, 255))
        closeWindow()
        endTime = time.time()

        if (endTime - startTime) < 5: 
            screen.blit(pg.font.SysFont(None, 48).render('THIS IS AN INCOMPLETE GAME', True, (255, 0, 0)), (20, 20))
        else:
            warningScreen = False
            titleScreen = True
